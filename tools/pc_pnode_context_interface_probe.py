#!/usr/bin/env python3
"""T087 probe: fixed-context interface product diagnostics."""
...
def context_coupling_seed_matrix() -> list[list[int]]:
    """Six-point fixed-context seed with a binary interface coupling."""
...
def _nested_p2_focus():
    return p_node([p_node([leaf(0), leaf(1)]), p_node([leaf(2), leaf(3)])])


def _t046_focus():
    return p_node([p_node([leaf(1), leaf(3)]), p_node([leaf(2), leaf(4)])])
...
def run_pnode_context_interface_probe(
    *,
    ...
    rows.append(
        _row(
            case="equal_context_nested_p2_control",
            D=equal_distance_instance(6),
            focus=_nested_p2_focus(),
            branch_order=(0, 1),
            context_before=(4,),
            context_after=(5,),
            max_branch_options=max_branch_options,
    ...
    rows.append(
        _row(
            case="context_coupling_seed_nested_p2",
    ...
    rows.append(
        _row(
            case="t046_matching_low_hub_context",
    ...
    for profile_name, profile in DISTANCE_PROFILES.items():
        ...
        rows.append(
            _row(
                case=f"handwritten_seed_context_{profile_name}",
                D=D,
                focus=_t085_focus(),
                branch_order=(0, 1, 2),
                context_before=(),
    ...
    summary = {
        ...
        "interpretation": (
            "diagnostic only; fixed context rows approximate an internal "
            "P-node interface with one outside embedding fixed"
        ),
    }
    return {
        "method": "t087_pnode_context_interface_product_probe",
        "parameters": {
            "max_branch_options": max_branch_options,
            "max_product_tuples": max_product_tuples,
            "max_examples": max_examples,
