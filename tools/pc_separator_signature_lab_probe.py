#!/usr/bin/env python3
"""T094 probe: separator signatures for partial P-node obligations."""
...
def _row(
    *,
    ...
    return {
        ...
        "branch_order_group_count": report["branch_order_group_count"],
        "branch_order_mixed_group_count": report["branch_order_mixed_group_count"],
        "support_boundary_branch_order_mixed_group_count": report[
            "support_boundary_branch_order_mixed_group_count"
        ],
        "fully_visible_branch_order_mixed_group_count": report[
            "fully_visible_branch_order_mixed_group_count"
        ],
        "separator_signature_group_count": report["separator_signature_group_count"],
        "separator_signature_mixed_group_count": report[
            "separator_signature_mixed_group_count"
        ],
        "support_boundary_separator_mixed_group_count": report[
            "support_boundary_separator_mixed_group_count"
        ],
        "fully_visible_separator_mixed_group_count": report[
            "fully_visible_separator_mixed_group_count"
        ],
        "removed_mixed_group_count": report["removed_mixed_group_count"],
        "support_boundary_removed_mixed_group_count": report[
            "support_boundary_removed_mixed_group_count"
        ],
        "nodes_with_branch_order_mixed_groups": report[
            "nodes_with_branch_order_mixed_groups"
        ],
        "nodes_with_separator_signature_mixed_groups": report[
            "nodes_with_separator_signature_mixed_groups"
        ],
        "fine_role_pattern_histogram": report["fine_role_pattern_histogram"],
        "branch_order_mixed_fine_role_pattern_histogram": report[
            "branch_order_mixed_fine_role_pattern_histogram"
        ],
        "separator_mixed_fine_role_pattern_histogram": report[
            "separator_mixed_fine_role_pattern_histogram"
        ],
        "max_branch_order_mixed_group_count": report[
            "max_branch_order_mixed_group_count"
        ],
        "max_separator_signature_mixed_group_count": report[
            "max_separator_signature_mixed_group_count"
        ],
        "node_summaries": [
            {
                "path": node["path"],
                ...
                "branch_order_group_count": node["branch_order_group_count"],
                "branch_order_mixed_group_count": node[
                    "branch_order_mixed_group_count"
                ],
                "separator_signature_group_count": node[
                    "separator_signature_group_count"
                ],
                "separator_signature_mixed_group_count": node[
                    "separator_signature_mixed_group_count"
                ],
                "removed_mixed_group_count": node["removed_mixed_group_count"],
                "branch_order_mixed_examples": node["branch_order_mixed_examples"],
                "separator_signature_mixed_examples": node[
                    "separator_signature_mixed_examples"
                ],
            }
            for node in interesting_nodes[:max_examples]
        ],
        "interpretation": "separator signature diagnostic only; not a solver",
    }
...
def run_probe(
    *,
    ...
    for row in rows:
        ...
        _merge_histogram(
            branch_order_mixed_fine_role_pattern_histogram,
            row["branch_order_mixed_fine_role_pattern_histogram"],
        )
        _merge_histogram(
            separator_mixed_fine_role_pattern_histogram,
            row["separator_mixed_fine_role_pattern_histogram"],
        )
    ...
    summary = {
        ...
        "rows_with_branch_order_mixed_groups": sum(
            1 for row in rows if row["branch_order_mixed_group_count"]
        ),
        "rows_with_separator_signature_mixed_groups": sum(
            1 for row in rows if row["separator_signature_mixed_group_count"]
        ),
        "rows_with_support_boundary_separator_mixed_groups": sum(
            1 for row in rows if row["support_boundary_separator_mixed_group_count"]
        ),
        ...
        "branch_order_group_count": sum(row["branch_order_group_count"] for row in rows),
        "branch_order_mixed_group_count": sum(
            row["branch_order_mixed_group_count"] for row in rows
        ),
        "support_boundary_branch_order_mixed_group_count": sum(
            row["support_boundary_branch_order_mixed_group_count"] for row in rows
        ),
        "fully_visible_branch_order_mixed_group_count": sum(
            row["fully_visible_branch_order_mixed_group_count"] for row in rows
        ),
        "separator_signature_group_count": sum(
            row["separator_signature_group_count"] for row in rows
        ),
        "separator_signature_mixed_group_count": sum(
            row["separator_signature_mixed_group_count"] for row in rows
        ),
        "support_boundary_separator_mixed_group_count": sum(
            row["support_boundary_separator_mixed_group_count"] for row in rows
        ),
        "fully_visible_separator_mixed_group_count": sum(
            row["fully_visible_separator_mixed_group_count"] for row in rows
        ),
        "removed_mixed_group_count": sum(row["removed_mixed_group_count"] for row in rows),
        "support_boundary_removed_mixed_group_count": sum(
            row["support_boundary_removed_mixed_group_count"] for row in rows
        ),
        "max_branch_order_mixed_group_count": max(
            (row["max_branch_order_mixed_group_count"] for row in rows),
            default=0,
        ),
        "max_separator_signature_mixed_group_count": max(
            (row["max_separator_signature_mixed_group_count"] for row in rows),
            default=0,
        ),
        "fine_role_pattern_histogram": dict(sorted(fine_role_pattern_histogram.items())),
        "branch_order_mixed_fine_role_pattern_histogram": dict(
            sorted(branch_order_mixed_fine_role_pattern_histogram.items())
        ),
        "separator_mixed_fine_role_pattern_histogram": dict(
            sorted(separator_mixed_fine_role_pattern_histogram.items())
        ),
        "interpretation": (
            "T094 refines T093 branch-order groups with the per-branch order "
            "of visible endpoint/witness roles. Remaining mixed groups are "
            "candidates for richer separator/context state."
        ),
    }
    return {
        "method": "t094_separator_signature_lab_probe",
        "sizes": sizes,
        "pc_trees": pc_trees,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/separator_signature_lab_probe.json"),
    )
