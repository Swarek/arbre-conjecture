...
def _ferrers_component_order(component, neighbors):
    ...
    for part_a_vertices, part_b_vertices in ((left, right), (right, left)):
        ...
        high_edges = {(a, b) for a in A_order for b in B_order if b in neighbors[a]}
...
def low_hub_component_ferrers_strong_ordering_report(D):
    """Detect component-wise binary low-hub Ferrers high graphs."""
    ...
    if len(off_diagonal_values) <= 1:
        order = tuple(range(n))
        return {
            "method": "low_hub_component_ferrers_strong_ordering_report",
            "n": n,
            "status": "empty_high_graph",
            "strong_ordering_exists": True,
            "low_value": off_diagonal_values[0] if off_diagonal_values else 0,
    ...
    A_blocks = []
    B_blocks = []
    for component in sorted(components, key=lambda item: min((*item[0], *item[1]))):
        component_order = _ferrers_component_order(component, neighbors)
        if component_order is None:
            return {
                **base,
                "status": "not_component_ferrers_high_graph",
                "strong_ordering_exists": None,
                "component_count": len(components),
            }
        A_block, B_block = component_order
        A_blocks.append(A_block)
        B_blocks.append(B_block)

    A_order = tuple(vertex for block in A_blocks for vertex in block)
    B_order = tuple(vertex for block in B_blocks for vertex in block)
    high_edges = {(a, b) for a in A_order for b in B_order if D[a][b] == high}
    if not _has_strong_ordering(A_order, B_order, high_edges):
        return {
            **base,
            "status": "component_ferrers_global_order_failed",
            "strong_ordering_exists": None,
            "component_count": len(components),
        }

    witness_order = tuple(hubs) + A_order + B_order
    return {
        **base,
        "status": "component_ferrers_strong_ordering_found",
        "strong_ordering_exists": True,
        "component_count": len(components),
        "part_a": A_order,
        "part_b": B_order,
